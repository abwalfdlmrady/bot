...
'''def server_cb_creator(job):
    ...
    keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data="back_admin")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    return reply_markup'''
...
def Reply_Kill(host, users):
    ...
    keyboard.append([InlineKeyboardButton("بازگشت↪️ to servers", callback_data="servers")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    return reply_markup
...
def server_cb_creator_user(job, data):
    ...
    keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data="back")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    return reply_markup
            ...
            if (status == "config"):
                ...
                if deposit >= 1000:
                    if deposit <= 1000000000:
                        ...
                        if (settings['nextpay'] == "on") and (settings['nextpay_address'] != "None"):
                            keyboard.append([InlineKeyboardButton("⚪️ درگاه نکست پی", callback_data=cb_nx)])
                        keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data='UWM')])
                        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_filtering(bot, query):
    ...
    if chat_id not in admin_id:
        query.answer("Access denied", show_alert=True)
        return
    keyboard = [[InlineKeyboardButton("بازگشت↪️", callback_data="back_admin")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_full(bot, query):
    ...
    if chat_id not in admin_id:
        query.answer("Access denied", show_alert=True)
        return
    keyboard = [[InlineKeyboardButton("بازگشت↪️", callback_data="SMT")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.edit_message_text(text="درحال انجام...")
    start = int(time())
    FS = ""
...
def call_MIOU(bot, query):
    ...
    user = data.split("$")[1]
    keyboard = [[InlineKeyboardButton("بازگشت↪️", callback_data='back')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    if check_exist_user(host, user) is True:
        try:
            port, username, password, panel, route_path, sshport, udgpw, remark = sshx.HOST_INFO(host)
...
def call_plisio(bot, query):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("بازگشت↪️", callback_data='ZBSHP')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    settings = get_settings()
...
def call_CUWPD(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = []
        Code = uuid4().hex[0:10]
        keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data='UWM_' + Code)])
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_ZUWPD(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("بازگشت↪️", callback_data='UWM_' + Code)]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
🔴 حتما بعد از تموم شدن روند پرداخت کافیه دکمه 'بررسی پرداخت' کلیک کنین."""
            ...
            keyboard = [[InlineKeyboardButton("بازگشت↪️", callback_data='UWM')]]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_CTPB(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = []
        Code = uuid4().hex[0:10]
        keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data='traffic_' + Code)])
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_ZTPB(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = [
            [InlineKeyboardButton("لینک پرداخت 🔗", url=invoice_url)],
            [InlineKeyboardButton("بازگشت↪️", callback_data='traffic_' + Code)]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_CC(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = []
        Code = uuid4().hex[0:10]
        keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data='buy_' + Code)])
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_ZP(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = [
            [InlineKeyboardButton("لینک پرداخت 🔗", url=invoice_url)],
            [InlineKeyboardButton("بازگشت↪️", callback_data='buy_' + Code)]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_XVPSS(bot, query):
    ...
    if check_domain_reached_maximum(host) is False:
        ...
        keyboard = [
            [InlineKeyboardButton("", callback_data=cb_custom), InlineKeyboardButton("تسویه حساب 💳", callback_data=cb_random)],
            [InlineKeyboardButton("<<", callback_data='buy')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        try:
            query.edit_message_text(text="برای انتقال به بخش بعد دکمه تسویه حساب رو بزنید✅", reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
        except:
            query.edit_message_text(text="📃برای انتقال به بخش بعد دکمه تسویه حساب رو بزنید✅", reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
    else:
...
def call_UPTXR(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = []
        Code = uuid4().hex[0:10]
        keyboard.append([InlineKeyboardButton("بازگشت↪️", callback_data='upgrade_' + Code)])
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_UPZPP(bot, query):
    ...
    if check_cache(chat_id) is False:
        ...
        keyboard = [
            [InlineKeyboardButton("لینک پرداخت 🔗", url=invoice_url)],
            [InlineKeyboardButton("بازگشت↪️", callback_data='upgrade_' + Code)]
        ]
        add_cache(chat_id, "upgrade")
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def call_SQerch(bot, query):
    ...
    keyboard = [[InlineKeyboardButton("بازگشت↪️", callback_data='back_seller')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.edit_message_text(text='خب حالا یه اسم یا حروف بفرستین مثلا ho (حداقل 2 حروف)', reply_markup=reply_markup)
