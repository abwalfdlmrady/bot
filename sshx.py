        ...
        if self.panel == "shahan":
            ...
            try:
                ...
                if s.status_code == 200:
                    ...
                    return f"هاست : <code>{IP}</code>\nپورت : <code>{PORT}</code>{dropbear}\nنام کاربری : <code>{uname}</code>\nرمز عبور : <code>{passw}</code>\n\nتعداد دستگاه قابل اتصال : {str(connection_limit)}\nتعداد روز : {str(days)}\nترافیک: {str(traffic)} گیگ "
                else:
            ...
            try:
                ...
                if s.status_code == 200:
                    ...
                    return f"هاست : <code>{self.host}</code>\nپورت : <code>{port}</code>\nنام کاربری : <code>{uname}</code>\nرمز عبور : <code>{passw}</code>\n\nتعداد دستگاه قابل اتصال : {str(connection_limit)}\nتعداد روز : {str(days)}\nتاریخ انقضا : {Date}\nترافیک: {str(traffic)} گیگ "
                else:
                    return "Error: Code " + str(s.status_code)
            except Exception as e:
                return "Error: " + str(e)
        ...
        if self.panel == "shahan":
            try:
                ...
                if "فعال" == status:
                    ...
                    status += "🔴"
                return f"هاست : <code>{self.ip}</code>\nپورت : <code>{port}</code>{drop}\nنام کاربری : <code>{self.uname}</code>\nرمز عبور : <code>{self.passwd}</code>\n\nتعداد دستگاه قابل اتصال : {str(self.connection_limit)}\nتعداد روز : {days}\nتاریخ انقضا : {self.Date}\nترافیک: {str(self.traffic)} گیگ \nUsage: {str(usage)}\nStatus: {status}{tuic}"
            except Exception as e:
                return "Error: " + str(e)
            ...
            try:
                port, udgpw, dropbear = self.Ports()
                if str(self.days) == "9999":
                    days = "بدون انقضا 🎗یا هنوز شروع نشده"
                else:
                    days = str(self.days)
                usage = self.usage + " GB"
                status = self.status
